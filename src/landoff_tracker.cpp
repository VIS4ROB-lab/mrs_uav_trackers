...
const std::array<const char*, 7> state_names = {
    ...
    "IDLING",       "LANDED",       "STOPPING_MOTION", "HOVERING",
...
class LandoffTracker : public mrs_uav_managers::Tracker {
 ...
 private:
  ...
  void callbackTakeoff(
      const std::shared_ptr<mrs_msgs::srv::Vec1::Request> request,
      std::shared_ptr<mrs_msgs::srv::Vec1::Response> response);
  void callbackLand(
      const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
  ...
  double current_heading_, current_vertical_direction_, current_vertical_speed_,
...
bool LandoffTracker::initialize(
    const rclcpp::Node::SharedPtr& node,
    std::shared_ptr<mrs_uav_managers::control_manager::CommonHandlers_t>
        common_handlers,
    std::shared_ptr<mrs_uav_managers::control_manager::PrivateHandlers_t>
  ...
  mrs_lib::ParamLoader param_loader_parent(common_handlers->parent_node,
                                           "LandoffTracker");
  ...
  if (!param_loader_parent.loadedSuccessfully()) {
    RCLCPP_ERROR(node_->get_logger(),
                 "[LandoffTracker]: Could not load all parameters!");
    return false;
  }
  ...
  private_handlers->param_loader->addYamlFile(
      ...
      "/config/private/landoff_tracker.yaml");
  private_handlers->param_loader->addYamlFile(
      ament_index_cpp::get_package_share_directory("mrs_uav_trackers") +
      "/config/public/landoff_tracker.yaml");

  private_handlers->param_loader->loadParam(
      "horizontal_tracker/horizontal_speed", _horizontal_speed_);
  private_handlers->param_loader->loadParam(
      "horizontal_tracker/horizontal_acceleration", _horizontal_acceleration_);

  private_handlers->param_loader->loadParam("vertical_tracker/vertical_speed",
                                            _vertical_speed_);
  private_handlers->param_loader->loadParam(
      "vertical_tracker/vertical_acceleration", _vertical_acceleration_);

  private_handlers->param_loader->loadParam("vertical_tracker/takeoff_speed",
                                            _takeoff_speed_);
  private_handlers->param_loader->loadParam(
      "vertical_tracker/takeoff_acceleration", _takeoff_acceleration_);

  private_handlers->param_loader->loadParam("vertical_tracker/landing_speed",
                                            _landing_speed_);
  private_handlers->param_loader->loadParam(
      "vertical_tracker/landing_acceleration", _landing_acceleration_);

  private_handlers->param_loader->loadParam("vertical_tracker/elanding_speed",
                                            _elanding_speed_);
  private_handlers->param_loader->loadParam(
      "vertical_tracker/elanding_acceleration", _elanding_acceleration_);

  private_handlers->param_loader->loadParam("heading_tracker/heading_rate",
                                            _heading_rate_);
  private_handlers->param_loader->loadParam("heading_tracker/heading_gain",
                                            _heading_gain_);

  private_handlers->param_loader->loadParam("main_timer_rate",
                                            _main_timer_rate_);

  private_handlers->param_loader->loadParam("landing_reference",
                                            _landing_reference_);

  private_handlers->param_loader->loadParam("max_position_difference",
                                            _max_position_difference_);

  private_handlers->param_loader->loadParam("takeoff_disable_lateral_gains",
                                            _takeoff_disable_lateral_gains_);
  private_handlers->param_loader->loadParam("takeoff_disable_lateral_gains_z",
                                            _takeoff_disable_lateral_gains_z_);
  ...
  RCLCPP_INFO(node_->get_logger(), "[LandoffTracker]: tracker_dt: %f",
              _tracker_dt_);
  ...
  profiler_ = mrs_lib::Profiler(common_handlers->parent_node, "LandoffTracker",
                                _profiler_enabled_);

  // | ------------------------ services ------------------------ |

  ss_takeoff_ = mrs_lib::ServiceServerHandler<mrs_msgs::srv::Vec1>(
      node_, "~/" + private_handlers_->name_space + "/takeoff",
      std::bind(&LandoffTracker::callbackTakeoff, this, std::placeholders::_1,
                std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);

  ss_land_ = mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(
      node_, "~/" + private_handlers_->name_space + "/land",
      std::bind(&LandoffTracker::callbackLand, this, std::placeholders::_1,
                std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_eland_ = mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(
      node_, "~/" + private_handlers_->name_space + "/eland",
      std::bind(&LandoffTracker::callbackELand, this, std::placeholders::_1,
                std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
...
void LandoffTracker::destroy(void) { timer_main_->stop(); }

//}

/* //{ activate() */

std::tuple<bool, std::string> LandoffTracker::activate(
...

  {
    ...
    RCLCPP_INFO(node_->get_logger(),
                "[LandoffTracker]: initial condition: x: %.2f, y: %.2f, z: "
                "%.2f, heading: %.2f",
                state_x_, state_y_, state_z_, state_heading_);
  }
...

  {
    ...
    horizontal_stop_dist =
        (horizontal_t_stop * current_horizontal_speed_) / 2.0;
...

  {
    ...
    vertical_stop_dist = current_vertical_direction_ *
                         (vertical_t_stop * current_vertical_speed_) / 2.0;
  }
...

  {
    ...
    RCLCPP_INFO(node_->get_logger(),
                "[LandoffTracker]: stopping goal: x: %.2f, y: %.2f, z: %.2f, "
                "heading: %.2f",
                goal_x_, goal_y_, goal_z_, goal_heading_);
  }
...
bool LandoffTracker::resetStatic(void) { return false; }

//}

/* //{ update() */

std::optional<mrs_msgs::msg::TrackerCommand> LandoffTracker::update(
    const mrs_msgs::msg::UavState& uav_state,
    [[maybe_unused]] const mrs_uav_managers::Controller::ControlOutput&
        last_control_output) {
  mrs_lib::Routine profiler_routine = profiler_.createRoutine("update");
  mrs_lib::ScopeTimer timer = mrs_lib::ScopeTimer(
      node_, "LandoffTracker::update", common_handlers_->scope_timer.logger,
...

  {
    ...
    position_output_.velocity.x =
        cos(current_heading_) * current_horizontal_speed_;
    position_output_.velocity.y =
        sin(current_heading_) * current_horizontal_speed_;
  ...
  if (_takeoff_disable_lateral_gains_ && taking_off_ &&
      uav_state_.pose.position.z < _takeoff_disable_lateral_gains_z_) {
...
const mrs_msgs::msg::TrackerStatus LandoffTracker::getStatus() {
  ...
  const bool hovering = current_state_vertical_ == HOVER_STATE &&
                        current_state_horizontal_ == HOVER_STATE;
  const bool idling = current_state_vertical_ == IDLE_STATE &&
                      current_state_horizontal_ == IDLE_STATE;

  // the (hovering && activate_as_the_first_tracker) part of the condition makes
  // sure that the IDLE will be reported even when hovering on the ground, just
  // before the takeoff service is called
...
LandoffTracker::enableCallbacks(
    const std::shared_ptr<std_srvs::srv::SetBool::Request>& request) {
...
const std::shared_ptr<std_srvs::srv::Trigger::Response>
LandoffTracker::switchOdometrySource(
  ...
  try {
    ...
    RCLCPP_ERROR_THROTTLE(
        ...
        "[LandoffTracker]: could not calculate the old UAV heading");
    got_headings = false;
  }
  ...
  try {
    ...
    RCLCPP_ERROR_THROTTLE(
        node_->get_logger(), *clock_, 1000,
        "[LandoffTracker]: could not calculate the new UAV heading");
    got_headings = false;
  }
...
const std::shared_ptr<std_srvs::srv::Trigger::Response> LandoffTracker::hover(
  ...
  auto [current_horizontal_speed, current_vertical_speed, current_heading,
        current_vertical_direction] =
      mrs_lib::get_mutexed(mutex_state_, current_horizontal_speed_,
                           current_vertical_speed_, current_heading_,
                           current_vertical_direction_);
  ...
  // --------------------------------------------------------------
  {
    ...
    current_horizontal_speed_ = sqrt(pow(uav_state.velocity.linear.x, 2) +
                                     pow(uav_state.velocity.linear.y, 2));
    current_vertical_speed_ = uav_state.velocity.linear.z;
    current_heading_ =
        atan2(uav_state.velocity.linear.y, uav_state.velocity.linear.x);
  }
  ...
  double vertical_stop_dist = current_vertical_direction *
                              (vertical_t_stop * current_vertical_speed) / 2.0;
...
const std::shared_ptr<std_srvs::srv::Trigger::Response>
LandoffTracker::startTrajectoryTracking(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>&
        request) {
  return nullptr;
}
...
const std::shared_ptr<std_srvs::srv::Trigger::Response>
LandoffTracker::stopTrajectoryTracking(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>&
        request) {
  return nullptr;
}
...
const std::shared_ptr<std_srvs::srv::Trigger::Response>
LandoffTracker::resumeTrajectoryTracking(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>&
        request) {
  return nullptr;
}
...
const std::shared_ptr<std_srvs::srv::Trigger::Response>
LandoffTracker::gotoTrajectoryStart(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>&
        request) {
  return nullptr;
}
...
LandoffTracker::setReference([[maybe_unused]] const std::shared_ptr<
...
const std::shared_ptr<mrs_msgs::srv::VelocityReferenceSrv::Response>
LandoffTracker::setVelocityReference(
    [[maybe_unused]] const std::shared_ptr<
...
const std::shared_ptr<mrs_msgs::srv::TrajectoryReferenceSrv::Response>
LandoffTracker::setTrajectoryReference(
    [[maybe_unused]] const std::shared_ptr<
...
void LandoffTracker::changeStateHorizontal(States_t new_state) {
  ...
  RCLCPP_INFO(node_->get_logger(),
              "[LandoffTracker]: Switching horizontal state %s -> %s",
              state_names.at(previous_state_horizontal_),
...
void LandoffTracker::changeStateVertical(States_t new_state) {
  ...
  RCLCPP_INFO(node_->get_logger(),
              "[LandoffTracker]: Switching vertical state %s -> %s",
              state_names.at(previous_state_vertical_),
...
void LandoffTracker::accelerateVertical(void) {
  // copy member variables
  auto [current_vertical_speed, state_z] =
      mrs_lib::get_mutexed(mutex_state_, current_vertical_speed_, state_z_);
  ...
  if (taking_off_) {
    ...
    if (used_speed > constraints.vertical_ascending_speed) {
      ...
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                           "[LandoffTracker]: saturating takeoff speed");
    }
    ...
    if (used_acceleration > constraints.vertical_ascending_acceleration) {
      ...
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                           "[LandoffTracker]: saturating takeoff acceleration");
    }
    ...
    if (elanding_) {
      ...
      if (used_speed > constraints.vertical_descending_speed) {
        ...
        RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                             "[LandoffTracker]: saturating landing speed");
      }
      ...
      if (used_acceleration > constraints.vertical_descending_acceleration) {
        ...
        RCLCPP_WARN_THROTTLE(
            node_->get_logger(), *clock_, 1000,
            "[LandoffTracker]: saturating landing acceleration");
      }
    }
  ...
  auto current_vertical_direction =
      mrs_lib::get_mutexed(mutex_state_, current_vertical_direction_);
  ...
  if (!elanding_ && !landing_) {
    if (fabs(state_z + stop_dist_z - goal_z) <
        (2 * (used_speed * _tracker_dt_))) {
      {
        std::scoped_lock lock(mutex_state_);
...
void LandoffTracker::decelerateVertical(void) {
  ...
  auto current_vertical_speed =
      mrs_lib::get_mutexed(mutex_state_, current_vertical_speed_);
...
void LandoffTracker::timerMain() {
  ...
  // copy member variables
  auto uav_state = mrs_lib::get_mutexed(mutex_uav_state_, uav_state_);
  auto [state_x, state_y, state_z, current_horizontal_speed,
        current_vertical_speed, current_heading, current_vertical_direction] =
      mrs_lib::get_mutexed(mutex_state_, state_x_, state_y_, state_z_,
                           current_horizontal_speed_, current_vertical_speed_,
                           current_heading_, current_vertical_direction_);
  auto [goal_x, goal_y, goal_z] =
      mrs_lib::get_mutexed(mutex_goal_, goal_x_, goal_y_, goal_z_);
  ...
  mrs_lib::ScopeTimer timer = mrs_lib::ScopeTimer(
      node_, "LandoffTracker::main", common_handlers_->scope_timer.logger,
      common_handlers_->scope_timer.enabled);
  ...
  if (taking_off_) {
    ...
    if (error_size > _max_position_difference_) {
      ...
      double future_state_x = state_x + cos(current_heading) *
                                            current_horizontal_speed *
                                            _tracker_dt_;
      double future_state_y = state_y + sin(current_heading) *
                                            current_horizontal_speed *
                                            _tracker_dt_;
      double future_state_z = state_z + current_vertical_direction *
                                            current_vertical_speed *
                                            _tracker_dt_;

      // if the step would lead to a greater control error than the threshold
      if (mrs_lib::geometry::dist(
              vec3_t(future_state_x, future_state_y, future_state_z),
              vec3_t(uav_x, uav_y, uav_z)) > error_size) {
        // set this to true... later, we will not update the model if this is
        // true, thus the tracker's motion will stop
        // => the tracker will wait for the controller
        takeoff_saturated = true;

        RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 100,
                             "[LandoffTracker]: position difference %.3f > "
                             "%.3f, saturating the motion. Reference: x=%.2f, "
                             "y=%.2f, z=%.2f, Odometry: %.2f, %.2f, %.2f",
                             error_size, _max_position_difference_,
                             future_state_x, future_state_y, future_state_z,
                             uav_x, uav_y, uav_z);
      }
    }
    ...
    if (last_control_output.diagnostics.ramping_up) {
      RCLCPP_INFO_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[LandoffTracker]: waiting for the controller to rampup");
      takeoff_saturated = true;
    }
  }
  ...
  if (current_state_horizontal_ == STOP_MOTION_STATE &&
      current_state_vertical_ == STOP_MOTION_STATE) {
    if (fabs(current_vertical_speed) <= 0.1 &&
        fabs(current_horizontal_speed) <= 0.1) {
      // if the current motion was stopped (the conditions above) but we still
      // have a goal (landing or taking off)
      // -> we should start accelerating towards the goal in the vertical
      // direction This is important, do not modify without testing, otherwise
      // your landing routine may crash into the ground while having large
      // lateral speed
      if (have_goal_) {
  ...
  if (current_state_vertical_ == STOPPING_STATE &&
      current_state_horizontal_ == STOPPING_STATE) {
    if (fabs(state_x - goal_x) > 1.0 || fabs(state_y - goal_y) > 1.0 ||
        fabs(state_z - goal_z) > 1.0) {
      RCLCPP_ERROR(
          node_->get_logger(),
          "[LandoffTracker]: distance to the goal is too large when STOPPING, "
          "this could have been caused by a race condition!");
    ...
    } else if (fabs(state_x - goal_x) < 0.1 && fabs(state_y - goal_y) < 0.1 &&
               fabs(state_z - goal_z) < 0.1) {
      {
        std::scoped_lock lock(mutex_state_);
  ...
  if (current_state_horizontal_ == LANDED_STATE &&
      current_state_vertical_ == LANDED_STATE) {
    {
      std::scoped_lock lock(mutex_state_);
  ...
  if (!takeoff_saturated) {
    {
      ...
      state_x_ +=
          cos(current_heading) * current_horizontal_speed * _tracker_dt_;
      state_y_ +=
          sin(current_heading) * current_horizontal_speed * _tracker_dt_;
      state_z_ +=
          current_vertical_direction * current_vertical_speed * _tracker_dt_;
    }
  }
  ...
  // compute the desired heading rate
  {
    ...
    if (fabs(state_heading_ - goal_heading_) <
        (2 * (_heading_rate_ * _tracker_dt_))) {
...
void LandoffTracker::callbackTakeoff(
    const std::shared_ptr<mrs_msgs::srv::Vec1::Request> request,
    std::shared_ptr<mrs_msgs::srv::Vec1::Response> response) {
  std::stringstream ss;
...
void LandoffTracker::callbackLand(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>
        request,
    std::shared_ptr<std_srvs::srv::Trigger::Response> response) {
  std::scoped_lock lock(mutex_main_timer_);
...
void LandoffTracker::callbackELand(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>
        request,
    std::shared_ptr<std_srvs::srv::Trigger::Response> response) {
  std::scoped_lock lock(mutex_main_timer_);
...
PLUGINLIB_EXPORT_CLASS(mrs_uav_trackers::landoff_tracker::LandoffTracker,
                       mrs_uav_managers::Tracker)
