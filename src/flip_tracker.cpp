...
bool FlipTracker::initialize(
  ...
  private_handlers->param_loader->addYamlFile(
      ...
      "/config/private/flip_tracker.yaml");
  private_handlers->param_loader->addYamlFile(
      ament_index_cpp::get_package_share_directory("mrs_uav_trackers") +
      "/config/public/flip_tracker.yaml");

  dynparam_mgr_->get_param_provider().copyYamls(
      private_handlers->param_loader->getParamProvider());

  private_handlers->param_loader->loadParam("activation_limits/max_velocity",
                                            _activation_max_velocity_);
  private_handlers->param_loader->loadParam(
      "activation_limits/max_acceleration", _activation_max_acceleration_);
  private_handlers->param_loader->loadParam(
      "activation_limits/max_heading_rate", _activation_max_heading_rate_);

  dynparam_mgr_->register_param("phases/z_acceleration/mode",
                                &drs_params_.z_mode,
                                mrs_lib::DynparamMgr::range_t<int>(0, 2));
  dynparam_mgr_->register_param("phases/z_acceleration/acceleration",
                                &drs_params_.z_acceleration,
                                mrs_lib::DynparamMgr::range_t<double>(0, 10.0));
  dynparam_mgr_->register_param("phases/z_acceleration/throttle",
                                &drs_params_.z_throttle,
                                mrs_lib::DynparamMgr::range_t<double>(0, 1.0));
  dynparam_mgr_->register_param("phases/z_acceleration/velocity_gain_from_rot",
                                &drs_params_.velocity_gain_from_rot,
                                mrs_lib::DynparamMgr::range_t<double>(0, 2.0));
  dynparam_mgr_->register_param("phases/flipping_pulse/attitude_rate",
                                &drs_params_.attitude_rate,
                                mrs_lib::DynparamMgr::range_t<double>(0, 30.0));
  dynparam_mgr_->register_param("phases/flipping_pulse/axis", &drs_params_.axis,
                                mrs_lib::DynparamMgr::range_t<int>(0, 1));
  dynparam_mgr_->register_param("phases/flipping_pulse/direction",
                                &drs_params_.direction,
                                mrs_lib::DynparamMgr::range_t<int>(0, 1));

  private_handlers->param_loader->loadParam(
      "phases/flipping_pulse/timeout_factor", _pulse_timeout_factor_);

  private_handlers->param_loader->loadParam("rampup/speed", _rampup_speed_);

  private_handlers->param_loader->loadParam("phases/recovery/duration",
                                            _recovery_duration_);
  private_handlers->param_loader->loadParam("phases/innertia/timeout_factor",
                                            _innertia_timeout_factor_);

  _pulse_timeout_ = _pulse_timeout_factor_ *
                    (FLIPPING_PULSE_STOP_TILT / drs_params_.attitude_rate);
  RCLCPP_INFO(node_->get_logger(), "initializing pulse timeout: %.4f s",
              _pulse_timeout_);
  _innertia_timeout_ =
      _innertia_timeout_factor_ *
      (((M_PI - FLIPPING_PULSE_STOP_TILT) + (M_PI - INNERTIA_PULSE_STOP_TILT)) /
       drs_params_.attitude_rate);
  RCLCPP_INFO(node_->get_logger(), "initializing inertia timeout: %.4f s",
              _innertia_timeout_);
  ...
  ss_flip_ = mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(
      node_, "~/" + private_handlers_->name_space + "/flip",
      std::bind(&FlipTracker::callbackFlip, this, std::placeholders::_1,
                std::placeholders::_2),
      cbkgrp_ss_);

  // | ------------------------ profiler ------------------------ |

  profiler_ = mrs_lib::Profiler(common_handlers->parent_node, "FlipTracker",
                                _profiler_enabled_);
...
std::tuple<bool, std::string> FlipTracker::activate(
    const std::optional<mrs_msgs::msg::TrackerCommand>& last_tracker_cmd) {
  std::stringstream ss;
...
bool FlipTracker::resetStatic(void) { return false; }

//}

/* //{ update() */

std::optional<mrs_msgs::msg::TrackerCommand> FlipTracker::update(
    const mrs_msgs::msg::UavState& uav_state,
    [[maybe_unused]] const mrs_uav_managers::Controller::ControlOutput&
        last_control_output) {
  auto current_state =
      mrs_lib::get_mutexed(mutex_current_state_, current_state_);
  auto drs_params = mrs_lib::get_mutexed(mutex_drs_params_, drs_params_);

  mrs_lib::Routine profiler_routine = profiler_.createRoutine("update");
  mrs_lib::ScopeTimer timer = mrs_lib::ScopeTimer(
      node_, "FlipTracker::update", common_handlers_->scope_timer.logger,
  ...
  double hover_throttle = mrs_lib::quadratic_throttle_model::forceToThrottle(
      common_handlers_->throttle_model, mass * g);
  ...
  switch (current_state) {
    ...
    case STATE_ACCELERATION: {
      ...
      if (rampup_active_) {
        ...
        if (std::abs((clock_->now() - rampup_start_time_).seconds()) >=
          ...
          RCLCPP_INFO_THROTTLE(node_->get_logger(), *clock_, 100,
                               "ramping up acceleration, %.4f", rampup_acc_);
        }
      ...
      if ((clock_->now() - state_change_time_).seconds() >=
          2 * z_acceleration_duration_) {
        RCLCPP_INFO(node_->get_logger(), "acceleration timeouted, recovering");
        mrs_lib::set_mutexed(mutex_current_state_, STATE_RECOVERY,
                             current_state_);
        state_change_time_ = clock_->now();
      }

      if (uav_state.velocity.linear.z > 0.95 * z_vel_gained_by_flipping_) {
        RCLCPP_INFO(node_->get_logger(), "z vel exceeded %.2f, flipping",
                    z_vel_gained_by_flipping_);
        mrs_lib::set_mutexed(mutex_current_state_, STATE_FLIPPING_PULSE,
                             current_state_);
        state_change_time_ = clock_->now();
      }
    ...
    case STATE_FLIPPING_PULSE: {
      ...
      if ((clock_->now() - state_change_time_).seconds() >= _pulse_timeout_) {
        ...
        RCLCPP_ERROR(node_->get_logger(),
                     "pulse phase took too long (%.4f s, timeout %.4f s), "
                     "startin recovery",
                     (clock_->now() - state_change_time_).seconds(),
                     _pulse_timeout_);

        state_change_time_ = clock_->now();

      } else if (tilt_angle > FLIPPING_PULSE_STOP_TILT) {
        mrs_lib::set_mutexed(mutex_current_state_, STATE_FLIPPING_INTERTIA,
                             current_state_);

        RCLCPP_INFO(
            node_->get_logger(), "pulse phase took %.4f s, (timeout %.4f s)",
    ...
    case STATE_FLIPPING_INTERTIA: {
      ...
      if ((clock_->now() - state_change_time_).seconds() >=
        ...
        RCLCPP_ERROR(node_->get_logger(),
                     "inertia phase took too long (%.4f s, timeout %.4f s), "
                     "startin recovery",
                     (clock_->now() - state_change_time_).seconds(),
                     _innertia_timeout_);
        ...
        RCLCPP_INFO(
            node_->get_logger(), "inertia phase took %.4f s, (timeout %.4f s)",
...
const std::shared_ptr<std_srvs::srv::Trigger::Response>
FlipTracker::switchOdometrySource(
...
const std::shared_ptr<std_srvs::srv::Trigger::Response> FlipTracker::hover(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>&
        request) {
  return nullptr;
}
...
const std::shared_ptr<std_srvs::srv::Trigger::Response>
FlipTracker::startTrajectoryTracking(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>&
        request) {
  return nullptr;
}
...
const std::shared_ptr<std_srvs::srv::Trigger::Response>
FlipTracker::stopTrajectoryTracking(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>&
        request) {
  return nullptr;
}
...
const std::shared_ptr<std_srvs::srv::Trigger::Response>
FlipTracker::resumeTrajectoryTracking(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>&
        request) {
  return nullptr;
}
...
const std::shared_ptr<std_srvs::srv::Trigger::Response>
FlipTracker::gotoTrajectoryStart(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>&
        request) {
  return nullptr;
}
...
FlipTracker::setReference([[maybe_unused]] const std::shared_ptr<
...
const std::shared_ptr<mrs_msgs::srv::VelocityReferenceSrv::Response>
FlipTracker::setVelocityReference(
    [[maybe_unused]] const std::shared_ptr<
...
const std::shared_ptr<mrs_msgs::srv::TrajectoryReferenceSrv::Response>
FlipTracker::setTrajectoryReference(
    [[maybe_unused]] const std::shared_ptr<
...
bool FlipTracker::callbackFlip(
  ...
  if (drs_params.z_mode == 0) {
    ...
    RCLCPP_INFO(node_->get_logger(), "accelerating with %.2f m/s^2",
                z_acceleration_acc_);
  } else if (drs_params.z_mode == 1) {
    z_acceleration_acc_ =
        (mrs_lib::quadratic_throttle_model::throttleToForce(
             common_handlers_->throttle_model, drs_params.z_throttle) /
         mass) -
        g;
    RCLCPP_INFO(node_->get_logger(),
                "accelerating with throttle %.2f => %.2f m/s^2",
                drs_params.z_throttle, z_acceleration_acc_);
  } else {
    std::stringstream ss;
  ...
  // calculate what velocity will the UAV gain while perfoming the flipping
  // maneuvre
  z_vel_gained_by_flipping_ = g * ((drs_params.velocity_gain_from_rot * M_PI) /
                                   drs_params.attitude_rate);

  // calculate how long do we have to accelerate to create a positive velocity
  // for the maneuvre
  z_acceleration_duration_ = z_vel_gained_by_flipping_ / z_acceleration_acc_;

  RCLCPP_INFO(node_->get_logger(),
              "z manouvre: acceleration: %.2f, duration: %.2f",
              z_acceleration_acc_, z_acceleration_duration_);
  ...
  mrs_lib::set_mutexed(mutex_current_state_, STATE_ACCELERATION,
                       current_state_);
...
bool FlipTracker::checkState(void) {
  ...
  try {
    heading_rate = mrs_lib::AttitudeConverter(uav_state_.pose.orientation)
                       .getHeadingRate(uav_state_.velocity.angular);
  } catch (...) {
  ...
  if (fabs(uav_state.velocity.linear.x) > _activation_max_velocity_ ||
      fabs(uav_state.velocity.linear.y) > _activation_max_velocity_ ||
  ...
  if (fabs(uav_state.acceleration.linear.x) > _activation_max_acceleration_ ||
      fabs(uav_state.acceleration.linear.y) > _activation_max_acceleration_ ||
...
PLUGINLIB_EXPORT_CLASS(mrs_uav_trackers::flip_tracker::FlipTracker,
