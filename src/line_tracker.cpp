...
bool LineTracker::initialize(
  ...
  if (!param_loader_parent.loadedSuccessfully()) {
    RCLCPP_ERROR(node_->get_logger(),
                 "[LineTracker]: Could not load all parameters!");
    return false;
  }
  ...
  private_handlers->param_loader->addYamlFile(
      ...
      "/config/private/line_tracker.yaml");
  private_handlers->param_loader->addYamlFile(
      ament_index_cpp::get_package_share_directory("mrs_uav_trackers") +
      "/config/public/line_tracker.yaml");
  ...
  private_handlers->param_loader->loadParam("tracker_loop_rate",
                                            _tracker_loop_rate_);
  ...
  RCLCPP_INFO(node_->get_logger(), "[LineTracker]: tracker_dt: %.2f",
              _tracker_dt_);
  ...
  profiler_ = mrs_lib::Profiler(common_handlers->parent_node, "LineTracker",
                                _profiler_enabled_);
...
void LineTracker::destroy(void) { timer_main_->stop(); }

//}

/* //{ activate() */

std::tuple<bool, std::string> LineTracker::activate(
    const std::optional<mrs_msgs::msg::TrackerCommand>& last_tracker_cmd) {
  std::stringstream ss;
...

  {
    ...
    if (last_tracker_cmd) {
      ...
      RCLCPP_INFO(node_->get_logger(),
                  "[LineTracker]: initial condition: x=%.2f, y=%.2f, z=%.2f, "
                  "heading=%.2f",
                  last_tracker_cmd->position.x, last_tracker_cmd->position.y,
                  last_tracker_cmd->position.z, last_tracker_cmd->heading);
      RCLCPP_INFO(node_->get_logger(),
                  "[LineTracker]: initial node_->get_logger(), condition: "
                  "x_rate=%.2f, y_rate=%.2f, z_rate=%.2f",
                  speed_x_, speed_y_, current_vertical_speed_);
      ...
      RCLCPP_WARN(node_->get_logger(),
                  "[LineTracker]: the previous command is not usable for "
                  "activation, using Odometry instead");
    }
  }
...

  {
    ...
    RCLCPP_INFO(node_->get_logger(), "[LineTracker]: setting z goal to %.2f",
                goal_z_);
  }
...
bool LineTracker::resetStatic(void) {
  if (!is_initialized_) {
    RCLCPP_ERROR(node_->get_logger(),
                 "[LineTracker]: can not reset, not initialized");
    return false;
  }

  if (!is_active_) {
    RCLCPP_ERROR(node_->get_logger(),
                 "[LineTracker]: can not reset, not active");
    return false;
  }
  ...
  try {
    ...
    RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "[LineTracker]: could not calculate the UAV heading");
    return false;
  }
...
std::optional<mrs_msgs::msg::TrackerCommand> LineTracker::update(
    const mrs_msgs::msg::UavState& uav_state,
    [[maybe_unused]] const mrs_uav_managers::Controller::ControlOutput&
        last_control_output) {
  mrs_lib::Routine profiler_routine = profiler_.createRoutine("update");
  mrs_lib::ScopeTimer timer = mrs_lib::ScopeTimer(
      node_, "LineTracker::update", common_handlers_->scope_timer.logger,
...
const std::shared_ptr<std_srvs::srv::Trigger::Response>
LineTracker::switchOdometrySource(
  ...
  try {
    ...
    RCLCPP_ERROR_THROTTLE(
        ...
        "[LineTracker]: could not calculate the old UAV heading");
    got_headings = false;
  }
  ...
  try {
    ...
    RCLCPP_ERROR_THROTTLE(
        node_->get_logger(), *clock_, 1000,
        "[LineTracker]: could not calculate the new UAV heading");
    got_headings = false;
  }
...
const std::shared_ptr<std_srvs::srv::Trigger::Response> LineTracker::hover(
  ...
  // --------------------------------------------------------------
  {
    ...
    current_horizontal_speed_ = sqrt(pow(uav_state_.velocity.linear.x, 2) +
                                     pow(uav_state_.velocity.linear.y, 2));
    current_vertical_speed_ = uav_state_.velocity.linear.z;
    current_heading_ =
        atan2(uav_state_.velocity.linear.y, uav_state_.velocity.linear.x);
  }
...
const std::shared_ptr<std_srvs::srv::Trigger::Response>
LineTracker::startTrajectoryTracking(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>&
        request) {
  return nullptr;
}
...
const std::shared_ptr<std_srvs::srv::Trigger::Response>
LineTracker::stopTrajectoryTracking(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>&
        request) {
  return nullptr;
}
...
const std::shared_ptr<std_srvs::srv::Trigger::Response>
LineTracker::resumeTrajectoryTracking(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>&
        request) {
  return nullptr;
}
...
const std::shared_ptr<std_srvs::srv::Trigger::Response>
LineTracker::gotoTrajectoryStart(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>&
        request) {
  return nullptr;
}
...
LineTracker::setConstraints(
  ...
  // this is the place to copy the constraints
  {
    ...
    _vertical_acceleration_ =
        request->constraints.vertical_ascending_acceleration;
...
LineTracker::setReference([[maybe_unused]] const std::shared_ptr<
...

  {
    ...
    RCLCPP_INFO(node_->get_logger(),
                "[LineTracker]: received new setpoint %.2f, %.2f, %.2f, %.2f",
...
const std::shared_ptr<mrs_msgs::srv::VelocityReferenceSrv::Response>
LineTracker::setVelocityReference(
    [[maybe_unused]] const std::shared_ptr<
...
const std::shared_ptr<mrs_msgs::srv::TrajectoryReferenceSrv::Response>
LineTracker::setTrajectoryReference(
    [[maybe_unused]] const std::shared_ptr<
...
void LineTracker::changeStateHorizontal(States_t new_state) {
  ...
  RCLCPP_DEBUG(node_->get_logger(),
               "[LineTracker]: Switching horizontal state %s -> %s",
               state_names[previous_state_horizontal_],
...
void LineTracker::changeStateVertical(States_t new_state) {
  ...
  // just for ROS_INFO
  RCLCPP_DEBUG(node_->get_logger(),
               "[LineTracker]: Switching vertical state %s -> %s",
               state_names[previous_state_vertical_],
...
void LineTracker::accelerateHorizontal(void) {
  ...
  auto [state_x, state_y, current_horizontal_speed] = mrs_lib::get_mutexed(
      mutex_state_, state_x_, state_y_, current_horizontal_speed_);
  ...
  if (sqrt(pow(state_x + stop_dist_x - goal_x, 2) +
           pow(state_y + stop_dist_y - goal_y, 2)) <
      (2 * (_horizontal_speed_ * _tracker_dt_))) {
    {
      std::scoped_lock lock(mutex_state_);
...
void LineTracker::accelerateVertical(void) {
  ...
  auto [state_z, current_vertical_speed] =
      mrs_lib::get_mutexed(mutex_state_, state_z_, current_vertical_speed_);
  ...
  if (fabs(state_z + stop_dist_z - goal_z) <
      (2 * (_vertical_speed_ * _tracker_dt_))) {
    {
      std::scoped_lock lock(mutex_state_);
...
void LineTracker::timerMain() {
  ...
  mrs_lib::ScopeTimer timer = mrs_lib::ScopeTimer(
      node_, "LineTracker::main", common_handlers_->scope_timer.logger,
      common_handlers_->scope_timer.enabled);

  auto [goal_x, goal_y, goal_z] =
      mrs_lib::get_mutexed(mutex_goal_, goal_x_, goal_y_, goal_z_);
  auto [state_x, state_y, state_z] =
  ...
  if (current_state_horizontal_ == STOPPING_STATE &&
      current_state_vertical_ == STOPPING_STATE) {
    if (fabs(state_x - goal_x) < 1e-3 && fabs(state_y - goal_y) < 1e-3 &&
        fabs(state_z - goal_z) < 1e-3) {
      {
        std::scoped_lock lock(mutex_state_);
...

  {
    ...
    state_x_ +=
        cos(current_heading_) * current_horizontal_speed_ * _tracker_dt_;
    state_y_ +=
        sin(current_heading_) * current_horizontal_speed_ * _tracker_dt_;
    state_z_ +=
        current_vertical_direction_ * current_vertical_speed_ * _tracker_dt_;
  }
...
PLUGINLIB_EXPORT_CLASS(mrs_uav_trackers::line_tracker::LineTracker,
